...
import editdistance
# Change from original: Two unneeded imports removed
import numpy as np
import sacrebleu
import scipy.stats
# Change from original: One unneeded import removed
...
# Change from original: line commented out
...
def squad(targets, predictions):
  ...
  Returns:
    dict with score_key: squad score across all targets and predictions
  """
  # Change from original: In the original line (commented out), the targets are 
  # not a list of list of strings, but rather list of strings. This interferes with the evaluation.
...
def trivia_qa(targets, predictions):
  ...
  targets = [qa_utils.normalize_trivia_qa(t) for t in targets]
